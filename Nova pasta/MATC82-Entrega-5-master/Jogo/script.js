...
function balaColidiu(){
	for(var i=0 ; i<balas.length ; i++){
		for(var j=0 ; j<inimigos.length ; j++){
			if(Math.sqrt(Math.pow((balas[i].getX() - inimigos[j].getX()), 2) + Math.pow((balas[i].getY() - inimigos[j].getY()), 2)) < 50){
...
function colisaoPlayer(item, indice, inimigos){
	if(Math.sqrt(Math.pow((jogador.getX() - inimigos[indice].getX()), 2) + Math.pow((jogador.getY() - inimigos[indice].getY()), 2)) < 100 / 2 && jogoEmAndamento == true){
		inimigos[indice].setSource("");
		drawExplosao(inimigos[indice]);
		//setInterval(drawExplosao(inimigos[indice]),100);
		/*$("canvas").drawImage({
...
var trackRight = 0; 
var trackLeft = 1; 
var frameWidth = spriteWidth/spriteCols; 
var frameHeight = spriteHeight/spriteRows;
var curFrame = 0;
var frameCount = 8;
var xSprt=0;
var ySprt=0;
...
explosao.src = "sprites/explosao2.png";
...
function drawExplosao(enemy){
	updateFrame();
	$("canvas").drawImage(explosao,srcX,srcY,frameWidth,frameHeight,enemy.x,enemy.y, 80, 80);
}
